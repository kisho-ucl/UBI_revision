...
{\title{\rev{自己教師あり学習を用いた \\ IMUデータによる産業現場作業認識の実環境評価}}
...
\rev{本研究では，IMUセンサを用いた作業認識における自己教師あり学習の活用について，
長期的な作業認識の特性に合わせて作業認識フレームワークを設計し，
その有効性を認識性能に加え，作業ごとの特性および訓練データ量との関係を分析した．}
物流倉庫における実作業データセットおよび OpenPack データセットを用いた評価実験では，
\rev{自己教師あり学習により事前学習したCNNと長期時系列モデリングを行うTransformer Encoderを組み合わせた作業認識モデルがベースラインを一貫して上回る認識性能が確認され，作業認識における自己教師あり学習を用いたラベルなしセンサデータ活用の実環境への適用可能性が示された．}
\end{abstract}
...
\rev{In this study, we investigate the use of self-supervised learning for task recognition using IMU sensor data. We adopt a task recognition framework tailored to long-duration work activities and analyze its effectiveness its effectiveness in terms of recognition performance, task characteristics, and training data size. The framework combines self-supervised pretraining for short-term motion representation with a Transformer-based model for capturing long-term temporal context.}
Experiments on a real-world logistics warehouse dataset and the OpenPack dataset show that {this framework consistently outperforms baseline methods. The results also indicate the effectiveness of leveraging unlabeled sensor data for task recognition and its applicability to real-world industrial environments.}
\end{eabstract}
...
\rev{このような実作業における動作の多様性やラベル収集コストといった課題に対し，
...
\item 実際の物流倉庫における実作業データおよびOpenPackデータセットを用いて，自己教師あり学習を活用した作業認識手法の性能を評価した．
...
自己教師あり学習の代表的な手法として，\rev{いくつかの枠組みが提案されている．}
\rev{補助タスクに基づく手法として，}Multi-task SSL\cite{saeed2019multi}は，
スケーリングやノイズ付加といったデータ拡張が入力信号に
適用されたかを識別する複数の補助タスクを同時に学習し，
センサデータに共通する有用な特徴表現を獲得する．
\rev{再構成に基づく手法として，}LIMU-BERT\cite{xu2021limu}は，
時系列信号の一部をマスクし，欠損部分を復元するタスクを通じて表現学習を行う．
このようなマスク信号復元に基づく学習により，
時系列データの周期性や時間的構造を捉えることが可能となる．
\rev{一方，対照学習に基づく手法は，センサベースの行動認識分野において特に活発に研究されている．
SimCLRHAR\cite{tang2020exploring}やCSSHAR\cite{khaertdinov2021contrastive}は，
コンピュータビジョン分野で提案されたSimCLR\cite{chen2020simple}をセンサデータに応用した手法であり，
...
以上より，SSL は短期動作の認識には有効であるものの，長期文脈の理解とユーザロバスト性を同時に満たした特徴表現をラベルなしデータから直接獲得することは容易ではない．そこで本研究では，長期構造とユーザロバスト性を同時に学習させるのではなく，自己教師あり学習によるロバストな基礎動作表現の獲得と，\rev{教師あり学習による作業文脈のモデリングを段階的に組み合わせた構成を用いて，
...
本研究では，\rev{長期的な作業認識モデルにおいて
ラベルなしセンサデータを活用するため，
短時間の基本動作に関する特徴抽出と，
長期的な時系列文脈のモデリングを組み合わせた構成を用いる．
具体的には，}作業を短時間に観測される基本動作の系列として捉え，その時間的構造から作業を推定する認識モデルを設計し，
\rev{短期の特徴抽出器のみを自己教師あり学習により事前学習する．}

\rev{本手法}の概要を図\ref{fig:overview}に示す．第1段階では，ラベルなしのセンサデータに対して自己教師あり学習を適用し，一般化可能な基本動作を捉えるCNNを事前学習する．
第2段階では，ラベル付きデータを用いてTransformer Encoder, MLPを含む長期時系列分類モデルを学習する．この際，CNNの重みは固定（もしくはfine-tuning）する．
この学習により，基本動作の時系列と作業との対応関係を効果的に学習\rev{することを目指す}．
\rev{本稿では，
このような事前訓練済みCNNとTransformer Encoderからなる作業認識モデルの構成をPreCNN-Tと表記する．
以上の構成を用いて，
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.92\linewidth, trim=10 37 10 15, bb=0 0 1407 520]{figure/overview.pdf} 
  \caption{\rev{特徴抽出器の自己教師あり学習を活用した長期作業認識モデル（PreCNN-T）}}
  \label{fig:overview}
\end{figure*}
...
% CSSHARなどが代表的な手法である．これらの手法では，SimCLRの枠組みに基づき，センサデータに対してノイズ付加やスケーリング，回転などの拡張を用いて対照学習を行うと，行動認識に有力な動作表現が得られることが報告されている．

\begin{table*}[t]
\centering
\caption{\rev{SimCLRによる事前学習に用いたデータ拡張の一覧}}
...
\subsubsection{\rev{SimCLRによる自己教師あり学習}}
\rev{本研究では，
対照学習を用いた自己教師あり学習の一つであるSimCLRの枠組みを用いて，
ラベルなしのセンサデータから短期的な動作特徴の学習を目指す．}

SimCLRに基づく対照学習手法では，ランダムな拡張で生成した2つの系列を共通のネットワークで特徴ベクトルに変換し，それらの特徴表現が近くなるように学習を進める．
具体的には，ミニバッチ内の各サンプル \(\boldsymbol{x}\) に対して，異なる2種類のランダムな変換を適用し，それぞれから得られた2つの系列 \(\tilde{\boldsymbol{x}}_i\), \(\tilde{\boldsymbol{x}}_j\) を正例ペアとする．  
この処理によって，バッチサイズが \(N\) の場合，\(2N\) 個の変換済みサンプルが得られる．  
それぞれのサンプルは共通のエンコーダで特徴ベクトル \(\boldsymbol{h}_i\), \(\boldsymbol{h}_j\) に変換された後，さらに小さな空間へ写像するプロジェクション層を通して，最終的な表現 \(\boldsymbol{z}_i\), \(\boldsymbol{z}_j\) となる．  
各正例ペア\((i, j)\)に対して，他の \(2N - 2\) 個の埋め込みを負例として扱い，以下のNT-Xent（Normalized Temperature-scaled Cross Entropy）損失\cite{chen2020simple} を最小化し，正例間の距離を縮め，負例とは遠ざけるように学習を行う．
...
\rev{この損失により，同一入力に由来する特徴表現は近づき，
異なる入力に由来する表現は遠ざかるように学習が進む．}

\subsubsection{データ拡張設定}
本研究における事前学習の目的は，短時間のセンサ系列から，
同一の基本動作に由来する信号が特徴空間上で近接するような
ロバストな動作特徴表現を獲得することである．
このため，センサに適用するデータ拡張が，
実環境で生じうる変動を一定程度反映していることが重要となる．
\rev{そこで本研究では，SimCLRHAR\cite{tang2020exploring}やCSSHAR\cite{khaertdinov2021contrastive}などの既存研究を参考に，
実環境で想定されるセンサ変動を反映する複数の拡張手法を選定し，
各拡張の性質に応じて適用確率を設定した．
具体的には，ノイズ付加や回転など比較的自然な変換を高い確率で適用し，
時間伸縮や順序入れ替えなど信号構造を大きく変える変換は補助的な拡張として低い確率で適用した．}
本研究で用いた拡張の一覧と適用確率を表~\ref{tab:augmentation}に示す．
% \begin{itemize}
...
\rev{本節では，自己教師あり学習を活用した作業認識モデル（PreCNN-T）の構成について，
...
産業現場作業認識に対する\rev{自己教師あり学習の有効性}を検証するため，
\rev{実環境で収集したデータを用いた評価実験を行う．}
まず，物流倉庫における実作業データを対象として，短期および長期の作業認識モデルを構築し，
自己教師あり学習の有無を含めて，
\rev{(1) 被験者単位交差検証による認識性能，
(2) 作業特性と認識精度の関係，
(3) 訓練データ量と認識精度の観点から評価する．}
さらに，\rev{作業構造や計測条件が異なる他の
公開データセットを用いた検証も行う．}
...
本研究では，物流倉庫入荷場における実作業に関するデータセットを収集し，\rev{限られたラベル量での作業認識について検証する．}
\rev{データ収集の際は}，物流倉庫の実際の作業員に，腰部にスマートフォンを装着した状態で業務を行ってもらい，
\rev{その間の3軸加速度および角速度データを収集して100 [Hz]にリサンプリングしてデータセットを作成した．}
...
\rev{対象とする作業ラベルは，商品が入荷されてから倉庫に格納されるまでの以下の人手作業である．
\begin{itemize}
  \item 検品(Inspect)：商品と注文書を照合する作業
  \item 仕分け(Sort)：商品を適切な場所に振り分ける作業
  \item 搬送(Transport)：荷物を載せた台車を運搬する作業
\end{itemize}
これらの作業は一定の役割分担が存在するものの，状況に応じて柔軟に交替され，作業が数分で切り替わる場合もあれば，数十分継続する場合もある.}
ラベル付きデータにおける各作業の出現割合は，図\ref{fig:label_distribution}に示す通りである．

\subsubsection{OpenPackデータセット}
\rev{多種多様な作業認識における自己教師あり学習の有効性や特性をより詳細に}確認するため，
公開データセットである OpenPack Dataset~\cite{yoshimura2024openpack} を用いた評価も行う．
OpenPack Dataset は，\rev{物流倉庫における梱包作業を対象として収集された}
マルチモーダル行動認識データセットであり，\rev{PackingやAssemble Boxなどの10種類の作業とそれらを構成するより詳細な27種類の動作}に関するラベル付きデータが16人の被験者から合計52時間が提供されている．
\rev{これらの作業は，数秒から数十秒程度の時間スケールで構成されており，本研究で対象とする作業認識問題と同様に，複数の動作が連続して構成される時系列構造を有する．}
...
これらを入力として SimCLR による自己教師あり学習を行う．

一方，作業認識モデルの訓練に用いるラベル付きセンサデータに対しては，
...
\subsection{\rev{比較手法}}
\rev{自己教師あり学習やモデル構成の違いが認識性能に与える影響}を評価するため，
入力長，事前学習の有無，および時系列モデリング手法の違いに基づいて，
複数のベースラインモデルを設定する．表~\ref{tab:model_arch} に本実験で用いる認識モデルの概要を示す．

CNN および \rev{PreCNN} は，短時間のセンサ系列を入力とし，
CNN によって動作特徴を抽出した後，MLP により分類を行うベースラインとして用いる．
\rev{PreCNN} では，CNNに対して\rev{SimCLR}による事前学習を適用する．

DeepConvLSTM（DCL）\cite{ordonez2016deep} は，
CNN による特徴抽出と LSTM による時系列モデリングを組み合わせた
代表的な行動認識モデルであり，長期依存関係を考慮する深層学習ベースラインとして採用する．

CNN-T および \rev{PreCNN-T} は，長時間のセンサ系列を入力とし，
CNN による特徴抽出と Transformer Encoder による長期文脈モデリングを
組み合わせた作業認識モデルである．
\rev{PreCNN-T}はCNNをSimCLRで事前学習した認識モデルである．

\rev{最後に，PreCNN, PreCNN-Tについては，
SimCLRにおけるデータ拡張設定において，3種の拡張(ノイズ追加，スケーリング，順序入れ替え)を用いる基本設定と，予備実験に基づき調整した6種の拡張(ノイズ追加，スケーリング，回転，時間シフト，時間歪み，順序入れ替え)を用いる調整後設定を比較する．}
...
\rev{さらに，訓練データ量と認識精度の関係を評価するため，
訓練データの一部のみを使って学習を行う実験を実施する．
具体的には，各被験者における各クラスのデータを対象に，
1\%，10\%，25\%，50\%，75\%，100\%の割合でサンプリングを行い，
クラス比率を維持したまま訓練データを構成した．
この設定により，ラベル付きデータ量が認識性能に与える影響を分析する．}

評価指標としては多クラス分類におけるF1スコアを用いる．
特に物流倉庫実作業データセットはクラス間の偏りが\rev{大きく，被験者ごとに一部クラスのサンプル数が極端に少ない場合があるため，}
...
\rev{表}~\ref{tab:cnn_arch} - \rev{表}\ref{tab:mlp_arch} に示す．
ベースラインモデルは，これらの構成要素の一部を用いており，パラメータを共有している．
...
各分割において，1人分を評価用，残り\rev{訓練用}とし，
さらに\rev{訓練データ}の 10\% を検証用データとしてハイパーパラメータ調整および early stopping に利用した．

CNN の事前学習にはSimCLRを用い，ラベルなしセンサデータを用いて
自己教師あり学習を行った．
損失関数には NT-Xent 損失を用い，
バッチサイズ 256，学習率 $1.0 \times 10^{-3}$ の設定のもと，
Adam\cite{kingma2014adam}最適化により最大 100 エポック学習した．
事前学習によって得られた CNN は，作業認識モデルにおいて特徴抽出器として利用する．


作業認識モデルの学習では，CNNを事前学習しない場合は，
CNNをランダム初期化した上でネットワーク全体をエンドツーエンドで学習した．3種類の拡張を用いる設定では，ノイズ追加（1.0），スケーリング, 順序入れ替え（各0.5）を適用し，6種類の拡張を用いる設定では，ノイズ追加（1.0), スケーリング, 回転, 時間シフト（各0.5），順序入れ替え, 時間歪み（各0.2）を加えた拡張を適用した．
...
\hline
CNN & 0.375 & 0.710 \\
PreCNN (拡張3種)  & 0.380 & 0.727 \\
PreCNN (拡張6種) & 0.405 & 0.749 \\
\hline
DCL & 0.420 & 0.705 \\
\hline
CNN-T & 0.363 & 0.694 \\
PreCNN-T (拡張3種)  & 0.454 & 0.793 \\
PreCNN-T (拡張6種) & \textbf{0.502} & \textbf{0.852} \\
\hline
\end{tabular}
\end{table}

\subsubsection{物流倉庫実作業データセット}
物流倉庫実作業データセットに対して，被験者間交差検証を行い，\rev{各モデル}の認識性能を評価した．\rev{表}\ref{tab:trusco_results}に各モデルについて全体の認識性能を示す．\rev{PreCNN} および \rev{PreCNN-T}は，事前学習をしない場合と比べてMacro-F1，Weighted-F1 の両指標が向上しており，
特に\rev{拡張6種のPreCNN-T}は両指標において最もスコアが高く，Macro-F1=0.502，Weighted-F1=0.852で3つの作業を分類した．また，拡張6種の設定は3種の場合と比較して一貫して高い性能を示した．以降では，PreCNN, PreCNN-Tは拡張6種で訓練されたモデルを指す．
...
これに対し，\rev{PreCNN}（図\ref{fig:confmat_trusco}(b)）では，仕分けと搬送作業の混同が低減しており，自己教師あり学習によって動作特徴の表現が改善されていることが確認できる．また，Transformer による時系列モデリングを導入した CNN-T（図\ref{fig:confmat_trusco}(c)）においても，作業文脈の考慮により誤分類が減少している．さらに，事前学習と時系列モデリングを組み合わせた PreCNN-T（図\ref{fig:confmat_trusco}(d)）では，さらに仕分けおよび搬送の混同が大きく改善し，双方において最も高い識別精度が得られている．

\rev{また，図\ref{fig:ratio_vs_f1_trusco}には訓練データ量と認識精度の関係を示す．
...
\subsubsection{OpenPackデータセット}
OpenPackデータセットを用いてLOOCVによる作業認識性能評価を行った．表\ref{tab:openpack_results}に各モデルの全体性能を示す．本データセットにおいても，\rev{PreCNN-T}の認識性能が最も高い認識性能を示し，Macro-F1=0.864，Weighted-F1=0.879で10個の作業を分類した．また，被験者ごとのWeighted-F1スコアの分布を示す図\ref{fig:op_scores}からも，\rev{PreCNN-T}は多くの被験者に対して安定して高いF1スコアを示していることが分かる．

\rev{また，作業ごとの持続時間に関する統計情報とF1スコアの関係をまとめたものを表\ref{tab:cnn_vs_precnn_t}に示す．
...
\hline
CNN & 0.568 & 0.569 \\
PreCNN (拡張3種) & 0.505 & 0.506 \\
PreCNN (拡張6種) & 0.562 & 0.564 \\
\hline
DCL & 0.780 & 0.803 \\
\hline
CNN-T & 0.801 & 0.821 \\
PreCNN-T (拡張3種) & 0.805 & 0.823 \\
PreCNN-T (拡張6種) & \textbf{0.864} & \textbf{0.879} \\
\hline
\end{tabular}
\end{table}
...
\rev{続いて，図\ref{fig:ratio_vs_f1_openpack}に訓練データ量と認識精度の関係を示す．
...
\subsection{考察}
評価結果より，本研究で用いた\rev{PreCNN-T}は，両データセットにおいて全体性能および被験者ごとの安定性の両面で最も高い性能を示した．
\rev{まず，性能向上の要因を作業ごとの分析から検討する．物流倉庫実作業データセットでは混同行列に基づき，類似した作業間の誤分類が緩和される傾向が見られた．一方，OpenPackデータセットにおいても，長期時系列モデルであるCNN-Tにより認識性能が全体的に向上する傾向が見られた．また，長時間かつばらつきの大きい作業においては比較的高い認識性能が維持される一方で，継続時間が短い作業では相対的に低いスコアとなる傾向が確認された．これは，短時間の作業では識別に有効な情報が限られるため，長期的な文脈だけでは十分に補えない可能性を示している．
また，PreCNN-Tでは，継続時間が最も短い作業において認識性能の改善幅が最も大きかったことから，
自己教師あり学習の導入により，エンドツーエンドでの学習と比べて短期的な動作特徴抽出の安定性が向上していると考えられる．}

\rev{また，SimCLRによる自己教師あり学習の際のデータ拡張の設定に関して，本実験では拡張の種類が多い設定で性能が向上する傾向が確認された．これは，多様な変換によりセンサ信号のばらつきに対して頑健な特徴表現が学習されたためと考えられる．
一方で，拡張の種類が少ない場合でも，事前学習後にfine-tuningを行うことで性能が補完される可能性が示唆される．
これらの関係については，拡張手法と学習戦略の相互作用の観点から，今後詳細な検討が必要である．}

\rev{次に，訓練データ量の観点から考察する．両データセットにおいて，訓練データ量が少ない条件では自己教師あり学習を導入したモデルの性能差が顕著であり，データ量の増加に伴いその差は徐々に縮小する傾向が確認された．特に，CNNのような短期入力モデルでは，訓練データ量の増加により自己教師あり学習なしでも十分に学習可能となるため，両者の差は比較的早期に縮小する傾向が見られる．一方で，CNN-Tのような長期時系列モデルでは，データ量が増加した場合でも自己教師あり学習を導入したPreCNN-Tが相対的に高い性能を維持する傾向が見られた．この結果は，長期時系列モデルにおいては特徴抽出と時系列モデリングの両方を最適化する必要があるため，事前学習による特徴表現の安定化が性能向上に大きく寄与する可能性を示唆している．}
...
この課題に対し，本研究では，
\rev{短期的な動作特徴の抽出と長期的な作業文脈のモデリングを分離して扱う構成（PreCNN-T）を用い，
...
\rev{今後は，データ拡張の設計や適用確率が認識性能に与える影響について，より体系的な分析を行うとともに，
拡張手法とfine-tuningを含む学習戦略の最適な組み合わせについて検討する必要がある．}
\rev{また，本研究で用いた短期特徴抽出と長期文脈モデリングの分離構成を拡張し，}時系列モデル側への自己教師あり学習の導入などを通じて，
