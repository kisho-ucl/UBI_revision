...
\subsubsection{\rev{SimCLRによる表現学習}}
\rev{本研究では，
...
\rev{そこで本研究では，既存研究で用いられている拡張を参考に，
...
さらに\rav{訓練データ}の 10\% を検証用データとしてハイパーパラメータ調整および early stopping に利用した．
...
PreCNN (拡張5種) & 0.405 & 0.749 \\
\hline
DCL & 0.420 & 0.705 \\
\hline
CNN-T & 0.363 & 0.694 \\
PreCNN-T (拡張3種)  & 0.454 & 0.793 \\
PreCNN-T (拡張5種) & \textbf{0.502} & \textbf{0.852} \\
\hline
\end{tabular}
\end{table}

\subsubsection{物流倉庫実作業データセット}
物流倉庫実作業データセットに対して，被験者間交差検証を行い，提案手法の認識性能を評価した．\rev{表}\ref{tab:trusco_results}に各モデルについて全体の認識性能を示す．PreCNN および PreCNN-Tは，事前学習をしない場合と比べてMacro-F1，Weighted-F1 の両指標が向上しており，
...
これに対し，PreCNN（図\ref{fig:confmat_trusco}(b)）では，仕分けと搬送作業の混同が低減しており，自己教師あり学習によって動作特徴の表現が改善されていることが確認できる．また，Transformer による時系列モデリングを導入した CNN-T（図\ref{fig:confmat_trusco}(c)）においても，作業文脈の考慮により誤分類が減少している．さらに，事前学習と時系列モデリングを組み合わせた PreCNN-T（図\ref{fig:confmat_trusco}(d)）では，さらに仕分けおよび搬送の混同が大きく改善し，双方において最も高い識別精度が得られている．
...
PreCNN (拡張5種) & 0.562 & 0.564 \\
\hline
DCL & 0.780 & 0.803 \\
\hline
CNN-T & 0.801 & 0.821 \\
PreCNN-T (拡張3種) & 0.805 & 0.823 \\
PreCNN-T (拡張5種) & \textbf{0.864} & \textbf{0.879} \\
\hline
\end{tabular}
\end{table}
