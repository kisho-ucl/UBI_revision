...
提案手法では，IMUデータを用いた作業認識において，自己教師あり学習を活用して少量ラベルでも高精度に予測可能な作業認識フレームワークを提案する．
評価の際は物流倉庫において収集した実データを用いて提案手法の有効性を詳細に検証する．
さらに，提案手法が特定の環境や作業内容に依存しないことを確認するため，
別のデータセットを用いた検証も行う．
...
\begin{itemize}
\item 産業現場における作業認識を対象とし， 短期的な動作特徴抽出と長期的な作業文脈モデリングを分離して扱う 作業認識フレームワーク（ActCNN-T）を提案した．
\item 物理的整合性を考慮したデータ拡張を用いた
自己教師あり学習を産業現場のセンサデータに導入し，被験者間差異や環境変動に対してロバストな動作表現が獲得可能であることを示した．
\item 実物流倉庫データセットおよび OpenPack データセットを用いた評価により， 提案手法がベースラインモデルを上回る性能を示し，提案手法の有効性を示した．
\end{itemize}
...
自己教師あり学習の代表的な手法として，以下のような手法が提案されている．
Multi-task SSL\cite{saeed2019multi} は，スケーリングやノイズ付加といったデータ拡張が入力信号に適用されたかを識別する複数の補助タスクを同時に学習し，センサデータに共通する有用な特徴表現を獲得する．
SimCLRHAR\cite{tang2020exploring, khaertdinov2021contrastive}やCSSHAR\cite{khaertdinov2021contrastive}は，同一の入力信号に対して異なるデータ拡張を施したペアを正例とし，それらの埋め込み表現の類似度を最大化する対照学習手法である．
LIMU-BERT\cite{xu2021limu} は，時系列信号の一部をマスクし，欠損部分を復元するタスクを通じて表現学習を行う再構成型の手法である．
マスク信号復元に基づく学習により，時系列データの周期性や時間的構造を捉えることが可能となる．
...
以上より，SSL は短期動作の認識には有効であるものの，長期文脈の理解とユーザロバスト性を同時に満たした特徴表現をラベルなしデータから直接獲得することは容易ではない．そこで本研究では，長期構造とユーザロバスト性を同時に学習させるのではなく，自己教師あり学習によるロバストな基礎動作表現の獲得と，教師あり学習による作業文脈のモデリングを段階的に組み合わせ，少量のラベルでも長期的な作業を高精度に認識できるフレームワークを提案する．


\section{提案手法}
\subsection{モデルの概要（ActCNN-T）}
本研究では，自己教師あり学習と教師あり学習を段階的に組み合わせた作業認識フレームワーク（ActCNN-T）を提案する．
本手法の基本的な考え方は，センサデータから作業に関する特徴を直接抽出するのではなく，作業を短時間に観測される基本動作の系列として捉え，その時間的構造から作業を推定する点にある．提案手法の概要を図\ref{fig:overview}に示す．第1段階では，ラベルなしのセンサデータに対して自己教師あり学習を適用し，一般化可能な基本動作を捉えるCNNを事前学習する．
第2段階では，ラベル付きデータを用いてTransformer Encoder, MLPを含む長期時系列分類モデルを学習する．この際，CNNの重みは固定（もしくはfine-tuning）する．
この学習により，基本動作の時系列と作業との対応関係を効果的に学習が可能となる．
...
\begin{table*}[t]
\centering
\caption{Hybrid Augmentation moduleの設計}
...
\subsubsection{物理的整合性を考慮したデータ拡張（CSSHAR+）}
本研究における事前学習の目的は，
短時間のセンサ系列から，
...
この目的を達成するためには，
センサに適用するデータ拡張が，
実環境で生じうる物理的変動を適切に再現していることが重要となる．
一方で，物理的整合性を大きく損なう変換であっても，
表現学習の観点では有効に機能する場合があることが示唆されている
\cite{tang2020exploring,khaertdinov2021contrastive}．

そこで本研究では，CSSHARの枠組みを拡張し，図\ref{fig:simclr}に示すように，データ拡張の性質に応じて適用確率を制御する Hybrid Augmentation Module を導入する． 具体的には，ノイズ付加や回転といった物理的整合性を保つ拡張 (Natural Transformation) を学習の基盤として高い確率で適用する．一方で，チャネルごとの時間伸縮やセグメント入れ替えなど，物理的意味を大きく変化させる構造的摂動 (Hard Transformation) については，学習の安定性を損なわないよう低い確率で補助的に適用する． これにより，物理的整合性を保った動作表現の学習と，表現の多様性を高める正則化効果とのバランスを最適化している． このような拡張戦略を採用した手法を CSSHAR+ と呼び，本研究で用いた拡張の一覧と適用確率を表~\ref{tab:augmentation} に示す．選定した拡張と適用確率は予備実験に基づいて設定されたものである．
% \begin{itemize}
...
\begin{figure*}[t]
  ...
  \caption{ActCNN-Tのアーキテクチャ} 
  \label{fig:architecture}
\end{figure*}



\subsubsection{NT-Xent損失の計算}
SimCLRに基づく対照学習手法では，ランダムな拡張で生成した2つの系列を共通のネットワークで特徴ベクトルに変換し，それらの特徴表現が近くなるように学習を進める．CSSHAR+でもこれと同様の流れで学習を行う．
...
この損失により，動作の識別性を保ちつつ，センサ値の変動に強い表現を学習できる．

\subsubsection{CSSHAR+の詳細}
CSSHAR+では，CNNからなるエンコーダと，
その出力を低次元空間へ写像するMLPから構成されるネットワークを用いる．
訓練対象となるエンコーダは，3.2節で述べた短時間ウィンドウごとに基本動作特徴を抽出するCNNである．

入力となる短時間ウィンドウ
$\boldsymbol{x}_i \in \mathbb{R}^{C \times L_{\mathrm{short}}}$
に対して，2種類のランダムなデータ拡張を適用し，
同一ウィンドウに由来する2つのビューを生成する．
これらのビューは，共通のCNNによって埋め込まれ，
それぞれ動作特徴ベクトル
$\boldsymbol{h}_i, \boldsymbol{h}_j \in \mathbb{R}^{d}$
として表現される．

得られた特徴ベクトルは，MLPからなるプロジェクションヘッドに入力され，
低次元の特徴ベクトル
$\boldsymbol{z}_i, \boldsymbol{z}_j \in \mathbb{R}^{d'}$
へと写像される．
本研究では，プロジェクションヘッドによって特徴次元を圧縮し，
対照学習に適した表現空間への再マッピングを行う．

これらの低次元特徴ベクトルに対して，
NT-Xent損失を用いた対照学習を行うことで，
同一入力に由来する特徴表現は近づき，
異なる入力に由来する表現は遠ざかるように学習が進む．
これによって，ラベルなしで動作特徴抽出器の訓練が可能となる．

\subsection{ActCNN-Tの詳細}
提案である作業認識モデルActCNN-Tのアーキテクチャの詳細を図\ref{fig:architecture}に示す．
...
産業現場における作業認識に対する提案手法の有効性を検証するため，
実環境で収集したデータを用いた評価実験を行う．
まず，物流倉庫における実作業データを対象として，
提案手法が実際の作業を高精度に認識できるかを評価する．
さらに，提案手法の汎用性を確認するため，異なるデータセットを用いた検証も行う．
...
\hline
CNN        & --          & Short (4\,s)  & -- \\
ActCNN   & \checkmark  & Short (4\,s)  & -- \\
\hline
DCL        & --          & Long (40\,s)  & LSTM \\
CNN-T      & --          & Long (40\,s)  & Transformer \\
ActCNN-T  & \checkmark  & Long (40\,s)  & Transformer \\
\hline
\end{tabular}
\end{table}
...
物流倉庫実作業データセットでは，
190時間分のラベルなしデータを用いてCSSHAR+によるCNN の事前訓練を行う．
...
CNN の事前学習にはCSSHAR+を用い，ラベルなしセンサデータを用いて
自己教師あり学習を行った．
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.95\linewidth, trim=0 0 0 0, clip, bb=0 0 856 189]{figure/trusco_scores.png} 
...
\subsection{評価結果}
\subsubsection{物流倉庫実作業データセット}
物流倉庫実作業データセットに対して，被験者を 1 名ずつテストデータとする被験者間交差検証を行い，提案手法の認識性能を評価した．図\ref{tab:trusco_results}に各モデルについて全体の認識性能を示す．ActCNN および ActCNN-Tは，事前学習をしない場合と比べてMacro-F1，Weighted-F1 の両指標が向上しており，
特にActCNN-Tは両指標において最もスコアが高く，Macro-F1=0.502，Weighted-F1=0.852で3つの作業を分類した．
...
DCL & 0.420 & 0.705 \\
CNN-T & 0.363 & 0.694 \\
ActCNN-T & \textbf{0.502} & \textbf{0.852} \\
\hline
\end{tabular}
\end{table}
...
これに対し，ActCNN（図\ref{fig:confmat_trusco}(b)）では，仕分けと搬送作業間の混同が低減しており，自己教師あり学習によって動作特徴の表現が改善されていることが確認できる．また，Transformer による時系列モデリングを導入した CNN-T（図\ref{fig:confmat_trusco}(c)）においても，作業文脈の考慮により誤分類が減少している．さらに，事前学習と時系列モデリングを組み合わせた ActCNN-T（図\ref{fig:confmat_trusco}(d)）では，さらに仕分けおよび搬送の混同が大きく改善し，双方において最も高い識別精度が得られている．


\begin{figure*}[t]
  \centering
  \includegraphics[width=0.95\linewidth, trim=0 0 0 0, clip, bb=0 0 856 189]{figure/openpack_scores.png} 
...
\subsubsection{OpenPackデータセット}
続いて，OpenPackデータセットを用いてLOOCVによる作業認識性能評価を行った．表\ref{tab:openpack_results}に各モデルの全体性能を示す．本データセットにおいても，提案手法であるActCNN-Tの認識性能が最も高く，Macro-F1=0.864，Weighted-F1=0.879で10個の作業を分類した．また，被験者ごとのWeighted-F1スコアの分布を示す図\ref{fig:op_scores}を見ると，ActCNN-Tは多くの被験者に対して安定して高いF1スコアを示していることが分かる．なお，本データセットにおいてはActCNN単体では単純なCNNとあまり性能が変化しなかった一方で，ActCNN-Tのように長期時系列モデルと組み合わせることで認識性能が改善している様子が確認された．

また，提案手法の実際の作業シーケンスに対する時系列予測の例として，ある被験者（U0207）における約5分間の認識結果を図\ref{fig:timeline}に示す．認識結果と正解ラベルを比べると，多少のラベル変動が確認されるものの，作業全体の推移という観点では，実際の作業シーケンスと整合した予測結果が得られていた．
...
DCL & 0.808 & 0.831 \\
CNN-T & 0.801 & 0.821 \\
ActCNN-T & \textbf{0.864} & \textbf{0.879} \\
\hline
\end{tabular}
\end{table}

\subsection{考察}
評価結果より，提案手法である ActCNN-T は，両データセットにおいて全体性能および被験者ごとの安定性の両面で最も高い性能を示した．ここでは性能向上の要因について考察する．
まず短期入力モデル（4 s）について，物流倉庫実作業データセットではCNNにCSSHAR+による自己教師あり学習を導入したActCNNで性能向上がみられ，事前学習が短時間の動作特徴の安定化に安定化に寄与していると示唆された．一方で，OpenPack データセットでは，CNN と ActCNN の性能差は限定的であり，短期入力条件においては，教師あり学習のみでもモデルが十分に学習可能であった可能性が考えられる．

これに対し，長期入力モデル（40 s）では，ActCNN-Tは，CNN-T を一貫して上回る性能を示しており，長期的な作業文脈を扱う条件において，動作特徴の事前学習が特に有効に機能することが分かる．
混同行列の分析からは，CNN 単体で顕著であった「仕分け（Sort）」と「搬送（Transport）」の混同が，事前学習または時系列モデリングの導入によってそれぞれ緩和され，両者を組み合わせた ActCNN-T では最も安定した識別が達成されていることが確認できる．
これらの結果は，短期的な動作特徴の表現学習と長期的な作業文脈のモデリングが相補的に機能することを示しており，本研究で採用した段階的な設計方針の妥当性を示唆している．
...
この課題に対し，本研究では，
短期的な動作特徴の抽出と長期的な作業文脈のモデリングを分離して扱う
作業認識フレームワーク ActCNN-T を提案した．
自己教師あり学習によってロバストな動作特徴を事前学習し，
その特徴系列を用いて作業レベルの時系列構造を学習し，
少量のラベルでも作業認識が可能な枠組みを構築した．
実物流倉庫データセットおよび OpenPack データセットを用いた評価の結果，
提案手法はベースラインを上回る認識性能と安定性を示した．
特に，事前学習による動作特徴の安定化と，
時系列モデリングによる作業文脈の考慮が相補的に機能することが確認された．
