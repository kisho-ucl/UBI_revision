...
\subsubsection{OpenPackデータセット}
続いて，OpenPackデータセットを用いてLOOCVによる作業認識性能評価を行った．表\ref{tab:openpack_results}に各モデルの全体性能を示す．本データセットにおいても，PreCNN-Tの認識性能が最も高い認識性能を示し，Macro-F1=0.864，Weighted-F1=0.879で10個の作業を分類した．また，被験者ごとのWeighted-F1スコアの分布を示す図\ref{fig:op_scores}を見ると，PreCNN-Tは多くの被験者に対して安定して高いF1スコアを示していることが分かる．なお，本データセットにおいてはPreCNN単体では単純なCNNとあまり性能が変化しなかった一方で，PreCNN-Tのように長期時系列モデルと組み合わせることで認識性能が改善している様子が確認された．

また，図\ref{fig:ratio_vs_f1_openpack}に訓練データ量と認識精度の関係を示す．
